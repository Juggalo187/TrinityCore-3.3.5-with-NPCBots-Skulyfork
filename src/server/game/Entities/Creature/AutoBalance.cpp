...
public:
    ...
    void ModifySpellDamageTaken(Unit* target, Unit* attacker, int32 &damage) override
    {
        ...
        if (target->IsCreature() && attacker->IsCreature() && !attacker->IsControlledByPlayer() && target->ToCreature()->IsTrigger())
            return;
...
bool IsBossNPC(Creature const* creature)
{
    return creature->isWorldBoss() || creature->IsDungeonBoss() || (creature->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_INSTANCE_BIND);
}

bool CanModifyCreatureAttributes(Creature const* creature)
{
    if (!creature || !creature->IsInWorld() || creature->IsControlledByPlayer() || creature->IsCritter() ||
        (creature->IsTrigger() && !IsBossNPC(creature)))
        return false;
...
private:
    ...
    void ModifyCreatureAttributes(Creature* creature, bool resetSelLevel = false)
    {
        ...
        scaledHealth = std::max<float>(1.0f, baseHealth * creatureABInfo->healthMultiplier);
        ...
        scaledMana = std::max<float>(baseMana ? 1.0f : 0.0f, baseMana * creatureABInfo->manaMultiplier);
